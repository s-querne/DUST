...
// [[Rcpp::export]]
List DUSTflist(NumericVector data, double penalty = 0) {
  ...
  for (int t = 2; t <= n; t++)
  {
    ...
    // DUSTflist
    while (i != validIndices.end() && nbConstraints > 0)
    {
      testValue = simpleTestFlist(t, *i, *j, valuesCumsum, costRecord);
